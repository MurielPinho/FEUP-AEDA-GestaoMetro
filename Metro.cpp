...
void Metro::readData()
{
  ...
  if (!mn) {
    ...
    while (getline(mn, line)) {
      ...
      M = new Manutencao(tr, tp, av, dt, (difftime(dt, dataAtual)) / 86400);
      Fila.push(*M);
    }
  }
...
void Metro::removeFuncionario() {
  ...
  BSTItrIn<Funcionario> itf(funcionarios);

  cout << "Funcionarios:" << endl << endl;
  cout << " ID |        Nome        |        Zona        |   Funcao   " << endl;

  while (!itf.isAtEnd()) {
    cout << itf.retrieve().getInfoTab() << endl;
    itf.advance();
  }
  cout << endl;
  cout << "Identificacao : ";
...
void Metro::dadosFuncionario() {
  ...
  while (!it.isAtEnd()) {
    cout << it.retrieve().getInfoTab() << endl;
    it.advance();
  }
  cout << endl;
}
...
void Metro::AddManutencao() {
  ...
  mdts->tm_min  = min;
  mdt           = mktime(mdts);
  cout << "Manutencao marcada para o dia:" << endl;
  cout << ctime(&mdt) << endl;
...
void Metro::PrintFila() {
  ...
  while (!Fila.empty()) {
    cout << Fila.top().getInformacao() << endl;
    copy.push_back(Fila.top());
    Fila.pop();
  }
